...
func (o *issueOperations) Update(ctx context.Context, request publicops.UpdateRequest) (publicops.UpdateResult, error) {
	...
	return RunTxResult(ctx, o.provider, func(ctx context.Context, uw UnitOfWork) (publicops.UpdateResult, string, error) {
		...
		// Recurrence parity with the classic update funnel (issueops
		// .updateIssueInTx): stopping a series clears its bounds, a pattern
		// with no start anchors on the due date, and the triple the update
		// would LAND validates against the same rule every create path
