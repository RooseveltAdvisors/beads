...
func ValidateScalarUpdates(ctx context.Context, tx DBTX, updates map[string]interface{}) error {
	...
	if raw, ok := updates["due_source"]; ok {
		value, isString := raw.(string)
		if !isString || !types.DueSource(value).IsValid() {
			return fmt.Errorf("%w: invalid due source %v (must be %s)",
				storage.ErrValidation, raw, types.ValidDueSourceNames())
		}
	}
	return nil
}
