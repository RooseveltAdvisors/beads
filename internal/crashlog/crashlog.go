...
func write(format string, args ...any) {
	...
	f, err := os.OpenFile(path, os.O_APPEND|os.O_CREATE|os.O_WRONLY, 0o600) //nolint:gosec // G304: path is Path(), the log dir under XDG state; the filename is a constant
	if err != nil {
		return
	}
	defer f.Close()
