...
// DueSource records where a bead's due date came from — the useful question
// once a bead has one is not "is there a date" but "did a human pick it".
//
// Whether a bead may be created WITHOUT a date is a separate question, owned
// by issueops.ValidateDueRequired and the `due.required` workspace switch.
...
const (
	...
	DueSourceBackfill DueSource = "backfill"
	// DueSourceRepeat marks a due date computed from a repeat pattern — either
	// the next instance spawned at close, or a missed occurrence rescheduled
	// forward by the due sweep.
	DueSourceRepeat DueSource = "repeat"
)

// IsValid reports whether s is empty (unset) or a known source.
func (s DueSource) IsValid() bool {
	switch s {
	case "", DueSourceExplicit, DueSourceDefault, DueSourceBackfill, DueSourceRepeat:
		return true
	}
	return false
}

// ValidDueSourceNames lists the accepted values, for error messages.
func ValidDueSourceNames() string {
	return "explicit, default, backfill, repeat"
}
...
func (i *Issue) ValidateRecurrence() error {
	if !i.DueSource.IsValid() {
		return fmt.Errorf("invalid due source %q (must be %s)", i.DueSource, ValidDueSourceNames())
	}
	if i.RepeatPattern == "" {
