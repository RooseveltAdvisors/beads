...
func TestValidateRecurrenceRejectsIncoherentRules(t *testing.T) {
	...
	tests := []struct {
		...
		{"end before start",
			...
			"before repeat_start"},
		{"unknown due source",
			Issue{DueSource: DueSource("guessed")}, "invalid due source"},
		{"over-length pattern",
...
func TestDueSourceValidity(t *testing.T) {
	for _, s := range []DueSource{"", DueSourceExplicit, DueSourceDefault, DueSourceBackfill, DueSourceRepeat} {
		if !s.IsValid() {
			t.Errorf("DueSource(%q).IsValid() = false, want true", s)
		}
	}
	if DueSource("made-up").IsValid() {
		t.Error("unknown DueSource reported valid")
	}
}
