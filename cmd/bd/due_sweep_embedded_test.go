...
func TestEmbeddedDueSweepFiresAndEscalates(t *testing.T) {
	...
	t.Run("a recurring bead follows its pattern instead of the grace push", func(t *testing.T) {
		dir, _, _ := bdInit(t, bd, "--prefix", "dr")
		issue := bdCreate(t, bd, dir, "Weekly report", "--type", "chore",
			"--due", pastDue(), "--repeat", "+1w")

		report := bdDueSweep(t, bd, dir)
		if !containsString(report.DueIDs, issue.ID) {
			t.Fatalf("due sweep did not fire recurring %s; report = %+v", issue.ID, report)
		}
		after := bdShow(t, bd, dir, issue.ID)
		if after.DueSource != "repeat" {
			t.Errorf("due_source = %q, want repeat (the pattern computed the new date, not the grace push)", after.DueSource)
		}
	})
